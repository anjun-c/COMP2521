...
int bstClosestHelper(struct node *t, int val, int closest);
...
int bstNumLeaves(struct node *t) {
	...
	return bstNumLeaves(temp->left) + bstNumLeaves(temp->right);
}
...
int bstRange(struct node *t) {
	...
	return bstMax(temp) - bstMin(temp);
}
...
struct node *bstDeleteLeaves(struct node *t) {
	...
	temp->left = bstDeleteLeaves(temp->left);
	temp->right = bstDeleteLeaves(temp->right);
	return t;
}
...
int bstClosest(struct node *t, int val) {
	...
	struct node *temp = t;
	if (temp == NULL) return -1;
	return bstClosestHelper(temp, val, temp->value);
}

int bstClosestHelper(struct node *t, int val, int closest) {
	struct node *temp = t;
	if (temp == NULL) return closest;
	if (abs(temp->value - val) < abs(closest - val)) closest = temp->value;
	if (val < temp->value) return bstClosestHelper(temp->left, val, closest);
	return bstClosestHelper(temp->right, val, closest);
}
...
// 	struct queue *new = malloc(2 * sizeof(struct queue *));
// 	new->node = node;
// 	new->next = NULL;
