...
int main(void) {
	...
	int *vals = malloc(maxVals); // incorrectly allocates memory int *vals = malloc(sizeof(int) * maxVals);
	if (vals == NULL) {
	...
	int numVals = 0;
	while (numVals < maxVals && scanf("%d", &vals[numVals]) == 1) { // program then accesses invalid array positions
		numVals++;
	}
