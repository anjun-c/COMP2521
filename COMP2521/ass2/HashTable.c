	...
	void HashTableInsert(HashTable table, int a, int b, int time) {
    if (table->numItems >= table->numSlots * MAX_LOAD_FACTOR) {
        resize(table);
    }
    int hashValue = hash(key, table->numSlots);
    while (!table->slots[hashValue].empty) {
        if (table->slots[hashValue].key == key) {
            table->slots[hashValue].value = value;
            return;
        }
        hashValue = (hashValue + 1) % table->numSlots;
    }
    table->slots[hashValue].key = key;
    table->slots[hashValue].value = value;
    table->slots[hashValue].empty = false;
    table->numItems++;
	}
	...
	void HashTableDelete(HashTable table, int key) {
		...
		if (table->slots[hashValue].empty) {
			return;
		}
		table->slots[hashValue].empty = true;
		table->numItems--;
		int nextSlot = (hashValue + 1) % table->numSlots;
		while (!table->slots[nextSlot].empty) {
			struct slot temp = table->slots[nextSlot];
			table->slots[nextSlot].empty = true;
			table->numItems--;
			HashTableInsert(table, temp.key, temp.value);
			nextSlot = (nextSlot + 1) % table->numSlots;
		}
	}
	...
	static inline unsigned int hash(int a, int b, int size) {
		return (a * 7 + b) % size;
	}
